...
def generarEmbedDeCola(nombreCola):
    ...
    if len(miembrosCola) > 1:
        ...
        # Solo si hay mas miembros, se llenan los a continuacion
        for persona in miembrosCola:
            if index == 1:
                index += 1
                continue
            mensajeCompleto = mensajeCompleto + str(index) + ") <@" + str(persona.id) + ">\n"
            index += 1

    # Creacion de mensaje embed
    mensajeEmbed = discord.Embed(
        title="Cola Actual: " + nombreCola,
        # url="https://realdrewdata.medium.com/",
        # description = "Para agregarte a esta cola solo debes tipear\n !queue add {nombreCola}",
...
@cliente.event
async def on_ready():
    print('We have logged in as {0.user}'.format(cliente))
...
async def on_message(message):
    ...
    # Comando para crear nueva cola (pisa a la anterior por ahora)
    ...
    # Comando para agregarse a la cola
